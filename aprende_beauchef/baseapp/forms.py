...
class PublishForm(forms.Form):
    ...
    price = forms.IntegerField(label='Precio estimado', min_value = 0, widget=forms.TextInput(attrs={'placeholder': '14000'}), required=True)
    modality = forms.ChoiceField(choices=[('pres', 'presencial'), ('rem', 'remota')], label='Modalidad', required=True)
    phone = forms.IntegerField(label='Teléfono de contacto', max_value=999999999, widget=forms.TextInput(attrs={'placeholder': '922224444'}), required=True)
    ...
    time_end = forms.TimeField(label='Hora fin', widget=forms.TextInput(attrs={'placeholder': 'HH:MM'}), required=True)